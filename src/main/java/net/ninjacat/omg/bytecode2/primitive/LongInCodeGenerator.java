...
public class LongInCodeGenerator<T> extends InCollectionCodeGenerator<T, Long> {

    private static final String VALUE_OF_DESC = Type.getMethodDescriptor(Type.getType(Long.class), Type.getType(long.class));

    public LongInCodeGenerator(final CodeGenerationContext context) {
        super(context);
    }

    @Override
    protected void boxIfNeeded(final Property<T, Long> property, final MethodVisitor method) {
        method.visitMethodInsn(Opcodes.INVOKESTATIC, Type.getInternalName(Long.class), "valueOf", VALUE_OF_DESC, false);
    }
     ...
     */
    @Override
    protected void createGetCollectionMethod(final String methodName, final Collection<Long> values) {
        ...
        generator.visitIntInsn(NEWARRAY, T_LONG);
        ...
        Stream.ofAll(values).forEachWithIndex((val, idx) -> {
            ...
            Codes.pushLong(generator, val);
            generator.visitInsn(LASTORE);
        });

        // convert array into stream and collect into set
        generator.visitMethodInsn(INVOKESTATIC, "java/util/Arrays", "stream", "([J)Ljava/util/stream/LongStream;", false);
        generator.visitMethodInsn(INVOKEINTERFACE, "java/util/stream/LongStream", "boxed", "()Ljava/util/stream/Stream;", true);
